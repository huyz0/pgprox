...
# asserts that every `m*-complete.sh`, `release-check.sh`, `fuzz.sh` and
...
[[ -f scripts/m10-complete.sh ]] \
  && ok "M10 has a gate of its own" \
  || fail "scripts/m10-complete.sh is missing, which is this milestone's own subject"
