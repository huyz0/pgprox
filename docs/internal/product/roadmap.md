...
| M87 | The mutants nobody has swept since M22 | in progress; fourteen of sixteen crates freshly swept, two real gaps fixed with tests and one memory-exhaustion mutant fixed with `debug_assert!` invariants after it took the machine from 30 GB free to swapping in under ten seconds |
...
`pgprox-testkit`, `pgprox-config`, `pgprox-observe`, `pgprox-admin` and
`pgprox-load` are freshly swept with every survivor checked against the
baseline. `pgprox-tls` and `pgprox-auth` each found a real gap of the same
shape, below. `pgprox` and `pgload` have not run yet.
...
out at the time. No new finding. Two crates and binaries remain: `pgprox`
and `pgload`.
